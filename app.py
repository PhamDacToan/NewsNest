...
'''def add_user_to_group(group_id):
    ...
    group = Group.query.get(group_id)
    if group:
        user_id = request.form.get('user_id')  # ID của người dùng được thêm vào
        user = User.query.get(user_id)
        
        if user:
            # Kiểm tra nếu nhân viên đã có trong nhóm chưa
            if user not in group.users:
                group.users.append(user)
                db.session.commit()
                flash(f'User {user.username} added to group {group.name} successfully!', 'success')
            else:
                flash(f'User {user.username} is already a member of this group.', 'danger')
        else:
...
def send_notification_to_user():
    ...
    users = User.query.all()
    notifications = Notification.query.all()
    return render_template('send_notification_to_user.html', users=users, notifications=notifications)
...
def send_notification_to_group():
    if request.method == 'POST':
        ...
        db.session.commit()
        flash('Notification sent to group!', 'success')
        return redirect(url_for('index'))
    
    groups = Group.query.all()
    notifications = Notification.query.all()
    return render_template('send_notification_to_group.html', groups=groups, notifications=notifications)
'''
...
def send_notification_to_group():
    if request.method == 'POST':
        # Kiểm tra nếu người dùng muốn tạo một thông báo mới
        if 'create_notification' in request.form:
            content = request.form['content']
            notification_type = request.form['type']
            category = request.form['category']
            file = request.files['file']
            ...
            # Gửi thông báo cho mỗi người dùng trong nhóm
            for user in group.users:
                user.notifications.append(notification)
